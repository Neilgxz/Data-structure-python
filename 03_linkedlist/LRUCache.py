...
class LRUCache:
    ...
    Author: Ben
    补充笔记：若用一个有序的单链表，表尾为越早访问。
    当一个新的数据被访问时，从链表头开始顺序遍历链表
    1. 如果此数据之前已经存在链表中了，我们遍历得到这个数据对应的结点，并将其从原来的位置删除，再插入到链表的头部
    2. 如果不在链表中且缓存未满，插入头部
    3. 如果缓存已满，把尾结点删除，新数据插入头部
    '''
    ...
    def __init__(self, capacity: int):
        ...
        # 哨兵：针对链表的插入，删除操作，需要对插入的第一个结点和删除最后一个结点的情况进行特殊处理。
        # 使用哨兵结点不存放数据且一直存在，统一代码逻辑。
